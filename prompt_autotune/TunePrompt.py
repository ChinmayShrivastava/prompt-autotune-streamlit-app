...
class TunePrompt:
    ...
    def __init__(
        ...
        # self.llm = llm
        self.powerllm = OpenAI("gpt-4o", max_tokens=2048, api_key=openapi_key) if powerllm is None else powerllm
        self.llm = OpenAI("gpt-4o", api_key=openapi_key) if llm is None else llm
        self.number_of_examples = number_of_examples
        if examples is None:
            examples = GenerateExamples(task, number_of_examples)(self.powerllm)
    ...
    def tune_prompt(self, cycle_no) -> None:
        if self.verbose:
            self.logger.info(f"Tuning prompt for cycle {cycle_no}")
